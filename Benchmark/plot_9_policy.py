...
def plot_dpi_ictf():
    ...
    p1 = plt.bar(ind, y_training, width, label = 'training', color="white", edgecolor="black")
    p2 = plt.bar(ind + width, y_privacy, width, label = '+ privacy policy', hatch="/", color="white", edgecolor="black")
    p3 = plt.bar(ind + width * 2, y_valid_100, width, label = '+ validation policy (100)', hatch='+', color="white", edgecolor = "black")
    p4 = plt.bar(ind + width * 3, y_valid_500, width, label = '+ validation policy (500)', hatch='x', color='white', edgecolor = 'black')

    axes = plt.gca()
    axes.set_ylim([0, 4050])

    plt.legend(ncol=2, columnspacing=0.2, fontsize=13, bbox_to_anchor=(0,1.02,1,0.2), loc="lower left", mode="expand", borderaxespad=0)

    plt.ylabel("time (s)",fontsize=15)
    plt.yticks(np.arange(0, 4050, 800), fontsize=12)
    plt.xticks([p + 1.5 * width for p in ind], ["$5000$", "$10000$", "$15000$", "$20000$"], fontsize=12)
