...
# All functions from private types

@base.vectorize
class ldpint(base.Instruction):
    r""" LDPINT i n
         Sets private integer register r_i to the immediate value n
         This instruction is vectorizable
    """
    __slots__ = []
    code = base.opcodes['LDINT']
...
@base.vectorize
class ldmpint(base.DirectMemoryInstruction, base.ReadMemoryInstruction):
    r""" LDMINT i n
         Assigns regint register r_i the value in memory R[n].
         This instruction is vectorizable
     """
    __slots__ = ["code"]
    code = base.opcodes['LDMPINT']
    arg_format = ['piw', 'int', 'p']

    def execute(self):
        self.args[0].value = program.mem_p[self.args[1]]

@base.vectorize
class ldmpinti(base.ReadMemoryInstruction):
    r""" LDMINTI i j 
         Assigns regint register r_i the value in memory R[r_j], where r_j is the j-th regint register.
         This instruction is vectorizable
     """
    code = base.opcodes['LDMPINTI']
    arg_format = ['piw', 'r', 'p']

    def execute(self):
        self.args[0].value = program.mem_p[self.args[1].value]

@base.vectorize
class ldmpintii(base.ReadMemoryInstruction):
    r""" LDMPINTI pi i j 
         Assigns pint register pi_i the value in memory P[r_j], where r_j is the j-th pint register.
         This instruction is vectorizable
     """
    code = base.opcodes['LDMPINTII']
    arg_format = ['piw', 'r', 'r']
...
@base.vectorize
class stmpint(base.DirectMemoryWriteInstruction):
    r""" STMPINT i n
         Sets memory P[n] to be the value in pint register pi_i.
         This instruction is vectorizable
     """
    __slots__ = ["code"]
    code = base.opcodes['STMPINT']
    arg_format = ['pi', 'int', 'p']

    def execute(self):
        program.mem_p[self.args[1]] = self.args[0].value

@base.vectorize
class stmpinti(base.WriteMemoryInstruction):
    r""" STMPINTI i j 
         Sets sint memory S[r_j] to be the value in sint register s_i, where r_j is the j-th regint register.
         This instruction is vectorizable
     """
    code = base.opcodes['STMPINTI']
    arg_format = ['pi', 'r', 'p']

    def execute(self):
        program.mem_p[self.args[1].value] = self.args[0].value

class private_input_pint(base.IOInstruction):
    r""" PRIVATE_INPUT_PINT i p m  
         Private input from player p on channel m assign result to pint pi_i
         Can only be executed in thread zero.
     """
    __slots__ = []
    code = base.opcodes['PRIVATE_INPUT_PINT']
    arg_format = ['piw', 'p', 'int']

class private_output_pint(base.IOInstruction):
    r""" PRIVATE_OUTPUT_PINT i p m  
         Private input from player p on channel m assign result to pint pi_i
         Can only be executed in thread zero.
     """
    __slots__ = []
    code = base.opcodes['PRIVATE_OUTPUT_PINT']
    arg_format = ['pi', 'p', 'int']

@base.vectorize
class addp(base.AddBase):
    r""" ADDPS i j k
         Adds a private register on party pi onto a secret one s_i=s_j+p_k.
         This instruction is vectorizable
     """
    __slots__ = []
    code = base.opcodes['ADDP']
    arg_format = ['piw', 'pi', 'pi', 'p']

@base.vectorize
class mulp(base.MulBase):
    r""" ADDPS i j k
         Adds a private register on party pi onto a secret one s_i=s_j+p_k.
         This instruction is vectorizable
     """
    __slots__ = []
    code = base.opcodes['MULP']
    arg_format = ['piw', 'pi', 'pi', 'p']

@base.vectorize
class addps(base.AddBase):
    r""" ADDPS i j k
         Adds a private register on party pi onto a secret one s_i=s_j+p_k.
         This instruction is vectorizable
     """
    __slots__ = []
    code = base.opcodes['ADDPS']
    arg_format = ['sw', 's', 'pi', 'p']

@base.vectorize
class addpc(base.AddBase):
    r""" ADDPS i j k
         Adds a private register on party pi onto a secret one s_i=s_j+p_k.
         This instruction is vectorizable
     """
    __slots__ = []
    code = base.opcodes['ADDPC']
    arg_format = ['piw', 'c', 'pi', 'p']
