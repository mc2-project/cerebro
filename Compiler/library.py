...
def load_private_mem(address, pid):
    return pint.load_mem(address, pid)

def load_mem(address, value_type, pid=None):
    if value_type in _types:
        value_type = _types[value_type]

    print("value_type is {}".format(value_type))

    if value_type is pint:
        if pid is None:
            for i in range(config.NUM_PARTIES):
                value_type.load_mem(address, i)
        else:
            value_type.load_mem(address, pid)
    else:
        value_type.load_mem(address)
    
@vectorize
def store_in_mem(value, address, pid=None):
    if isinstance(value, int):
        value = load_int(value)
    try:
        if isinstance(value, pint):
            assert(pid is not None)
            value.store_in_mem(address, pid)
        else:
