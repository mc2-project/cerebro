...
def plot_dpi_ictf():
    ...
    axes.set_ylim([55000, 255000])
    # axes.set_xlim([0,700])

    plt.legend(ncol=2, columnspacing=0.2, fontsize=15, loc = [0.42, 0.40])

    plt.ylabel("Throughput (triple/s)",fontsize=15)
    plt.yticks(np.arange(55000,255000,40000), fontsize=15)
    plt.xlabel("Number of parties", fontsize=15)
    xts = [2, 4, 6, 8, 10, 12]
    plt.xticks(xts, ["$2$", "$4$", "$6$", "$8$", "$10$", "$12$"], fontsize=15)

    fig = plt.gcf()
    fig.set_size_inches(6, 4, forward=True)

    pp = PdfPages('fileaccesslatency.pdf')
